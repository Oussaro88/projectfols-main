...
public class StateAttackMagicLightningStorm : StateAttackMagic
{
        ...
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("Spell") && !once2)
        {
            if (anim.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.5 && anim.GetCurrentAnimatorStateInfo(0).normalizedTime < 1)
            {
                ...
                if (isPooling)
                {
                    ...
                    if (isRandom)
                    {
                        SpawnSpellStorm(enemyBehaviour, true, pathPointList.Length);
                    }
                    else //Focus Attack
                    {
                        if (foundPlayerID < 4)
                        {
                            SpawnSpellStorm(enemyBehaviour, false, 9);
                        }
                        else if (foundPlayerID > 4)
                        {
                            SpawnSpellStorm(enemyBehaviour, false, 6);
                        }
                        else if (foundPlayerID == 4)
                        {
                            SpawnSpellStorm(enemyBehaviour, false, 12);
                        }
                    }
    ...
    public void SpawnSpellStorm(EnemyBehaviour enemyBehaviour, bool rand, int max)
    {
        if (rand)
        {
            do
            {
                ...
                magic = enemyBehaviour.poolingManager.callLightningStorm();
                magic.SetActive(true);
