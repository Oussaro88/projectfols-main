...
public class StateAttackMagicFireMeteor : StateAttackMagic
{
        ...
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("Spell") && !once2)
        {
            if (anim.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.5 && anim.GetCurrentAnimatorStateInfo(0).normalizedTime < 1)
            {
                ...
                if (isPooling)
                {
                    ...
                    //magic = enemyBehaviour.poolingManager.callFireMeteor();
                    if (isRandom)
                    {
                        SpawnSpellMeteor(enemyBehaviour, true, pathPointList.Length);
                    }
                    else //Focus Attack
                    {
                        if (foundPlayerID < 4)
                        {
                            SpawnSpellMeteor(enemyBehaviour, false, 9);
                        }
                        else if (foundPlayerID > 4)
                        {
                            SpawnSpellMeteor(enemyBehaviour, false, 6);
                        }
                        else if (foundPlayerID == 4)
                        {
                            SpawnSpellMeteor(enemyBehaviour, false, 12);
                        }
                    }
    ...
    public void SpawnSpellMeteor(EnemyBehaviour enemyBehaviour, bool rand, int max)
    {
        if (rand)
        {
            do
